...
/*   Updated: 2022/06/25 19:35:14 by myukang          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	check_redirection_output(t_dlst *tok_lst)
{
	while (tok_lst)
	{
		if (GET_TOKEN_TYPE(tok_lst) >= 8 && GET_TOKEN_TYPE(tok_lst) <= 10)
			return (1);
...
t_dlst	*make_iolst(t_data *data)
{
	...
	{
		while (tok_lst && (GET_TOKEN_TYPE(tok_lst) < 8 || GET_TOKEN_TYPE(tok_lst) > 10))
			tok_lst = tok_lst->next;
		if (tok_lst)
		{
